...
def all_issues(token:str, repo:str)->list[dict[str,Any]]:
    ...
    while True:
        ...
        if not isinstance(chunk, list):
            raise TypeError("issues endpoint returned non-list")
        out += chunk
        if len(chunk) < 100:
            return out
        page += 1
...
def label(item: dict[str, Any]) -> str:
    ...
    if item["type"] in {"feature", "epic"}:
        return "enhancement"
    if item["type"] == "chore" and item["area"] in {"docs", "documentation"}:
...
def main() -> int:
    ...
    encoded = "".join(path.read_text().strip() for path in sorted((root / ".github/product-backlog").glob("catalogue.*.b64")))
    catalogue = json.loads(gzip.decompress(base64.b64decode(encoded)))
    existing = all_issues(token, repo)
    by_marker = {}
    by_title = {}
    for issue in existing:
        if found_marker := marker(issue.get("body")):
            by_marker[found_marker] = issue
        normalized = norm(issue.get("title", ""))
        if normalized:
            by_title.setdefault(normalized, issue)
    numbers = {key: int(value["number"]) for key, value in by_marker.items()}
    created = []
    skipped = []
    errors = []
    for item in catalogue:
        ...
        if found:
            numbers[item["id"]] = int(found["number"])
            skipped.append(found["number"])
            print(f"SKIP #{found['number']}: {item['title']}")
            continue
        try:
            ...
            numbers[item["id"]] = int(issue["number"])
            by_marker[item["id"]] = issue
            by_title[norm(item["title"])] = issue
            created.append({"number": issue["number"], "title": item["title"], "url": issue["html_url"]})
