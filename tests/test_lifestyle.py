...
def test_high_carb_on_deadlift_days() -> None:
    for day in (1, 4):
...
def test_moderate_carb_on_leg_days() -> None:
    for day in (3, 6):
        guidance = lifestyle.daily_guidance(day, is_rest=False)
        assert guidance.carb_tier == "moderate"


def test_low_carb_on_upper_and_rest_days() -> None:
    for day in (2, 5):
...
def test_zone2_only_on_light_and_rest_days() -> None:
...
def test_no_running_or_stairmaster_mentioned_for_cardio_days() -> None:
    cardio = lifestyle.daily_guidance(5, is_rest=False).cardio
    assert "No stair-master or running" in cardio


def test_protein_target_from_bodyweight() -> None:
...
def test_protein_target_absent_without_weight() -> None:
...
def test_training_names_the_session() -> None:
...
def test_training_on_rest_day() -> None:
