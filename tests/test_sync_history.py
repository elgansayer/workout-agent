...
def test_get_all_workouts_returns_none(tmp_path) -> None:
...
def test_get_all_workouts_returns_empty_list(tmp_path) -> None:
    db_path = _new_temp_db(tmp_path)
...
def test_sync_all_rebuilds_and_saves_workouts(tmp_path) -> None:
    db_path = _new_temp_db(tmp_path)
...
def test_sync_all_respects_user_id(tmp_path) -> None:
    db_path = _new_temp_db(tmp_path)
    with patch("sync_history.get_all_workouts", return_value=_SAMPLE_WORKOUTS):
...
def test_sync_all_workout_missing_start_time(tmp_path) -> None:
    db_path = _new_temp_db(tmp_path)
    ...
    with patch("sync_history.get_all_workouts", return_value=workouts):
        result = sync_history.sync_all("fake-key", db_path)
    assert result == {"workouts_found": 1, "processed": 1}


def test_sync_all_parse_failure_skips_progress(tmp_path) -> None:
...
def test_sync_all_no_date_uses_none(tmp_path) -> None:
    db_path = _new_temp_db(tmp_path)
    workouts: list[dict[str, object]] = [
        {
            "id": "W1",
...
def test_rebuild_clears_tables(tmp_path) -> None:
