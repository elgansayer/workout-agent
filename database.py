...
def get_progress_history(
    ...
    # ⚡ Bolt: Use SQL ROW_NUMBER window function to only fetch the limited dataset
    # from the database, preventing massive memory allocation over time as the
    # exercise log grows.
    with _connect(db_path) as conn:
        rows = conn.execute(
            """
            ...
            WHERE rn <= ?
            ORDER BY exercise_name ASC, rn DESC
            """,
            (limit_per_exercise,)
        ).fetchall()
