...
- **Hourly dead-code sweep re-verified.** `dead_code_sweep.py` executed
  ...
  (zero orphans), import-sanity (all reachable). Issue #491 sweep complete
  — no newly orphaned or truly-dead modules found. `programme_inference.py`
  and `hevy_reader.py` remain wired via `webapp/app.py`'s
